...
public class Point implements Comparable<Point> {
  ...
  public int compareTo(Point that) {
    if (this.y > that.y) return 1; else if (this.y < that.y) return -1; else {
      if (this.x > that.x) return 1; else if (this.x < that.x) return -1;
    }
    return 0;
  }
  ...
  public Comparator<Point> slopeOrder() {
    return new Comparator<Point>() {
      ...
      public int compare(Point p1, Point p2) {
        if (slopeTo(p1) > slopeTo(p2)) return 1; else if (
          slopeTo(p1) < slopeTo(p2)
